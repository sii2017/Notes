...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow)
{
	...
	hwnd= CreateWindow(szAppName, TEXT("Show DIB #2"), WS_OVERLAPPEDWINDOW, 
...
int ShowDib(HDC hdc, BITMAPINFO* pbmi, BYTE* pBits, int cxDib, int cyDib, int cxClient, int cyClient, WORD wShow)
{
	...
	case IDM_SHOW_NORMAL:
		return SetDIBitsToDevice(hdc, 0,0, cxDib, cyDib, 0,0, 0, cyDib, 
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	static BYTE* pBits;
	static DOCINFO di= {
		sizeof(DOCINFO), TEXT("ShowDib2:Printing:")
	};
