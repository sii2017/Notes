...
void DisplayDibHeaders(HWND hwnd, TCHAR * szFileName)
{
	...
	Printf(hwnd, TEXT("File:%s\r\n\r\n"), szFileName);	//\r 表示回到行首，并用跟在 \r 后面要显示的内容（如果有的话）进行覆盖。 \n 就是换行。
	//Open the file
	...
	//Display BITMAPFILHEADER structure
	...
	//Determiine which infomation structure we have
...
LRESULT CALLBACK WndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		hwndEdit= CreateWindow(TEXT("edit"), NULL, WS_CHILD| WS_VISIBLE| WS_BORDER| 
			WS_VSCROLL| WS_HSCROLL| ES_MULTILINE| ES_AUTOVSCROLL| ES_READONLY, 
