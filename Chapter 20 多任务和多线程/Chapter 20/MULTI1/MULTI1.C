...
//windows1: Display incresing sequence of numbers
...
LRESULT APIENTRY WndProc3(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_TIMER:
		...
		TextOut(hdc, 0, iLine*cyChar, szBuffer, wsprintf(szBuffer, "%d", iNum));
		ReleaseDC(hwnd, hdc);
		iTemp= iNum;
		iNum= iNext;
		iNext+= iTemp;
...
//windows 4: Display circles of random radii
LRESULT APIENTRY WndProc4(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_TIMER:
		...
		hdc= GetDC(hwnd);
		Ellipse(hdc, (cxClient-iDiameter)/2, (cyClient- iDiameter)/2,
			(cxClient+iDiameter)/2, (cyClient+iDiameter)/2);
		ReleaseDC(hwnd, hdc);
		return 0;
	}
	return DefWindowProc(hwnd, message, wParam, lParam);
		...
		static WNDPROC ChildProc[]= {
			...
			case WM_CREATE:
				...
				for(i=0; i<4; i++)
				{
					...
					hwndChild[i]= CreateWindow(szChildClass[i], NULL, WS_CHILDWINDOW| WS_BORDER| WS_VISIBLE, 0, 0,0,0,
