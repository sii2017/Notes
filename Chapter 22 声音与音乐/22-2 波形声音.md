...
**参考RECORD2**    
RECORD2只使用两个MCI函数调用，其中最重要的调用如下：   
```c
error = mciSendCommand (wDeviceID, message, dwFlags, dwParam)   
```    
第一个参数是设备的识别数字（ID），可以按句柄来使用ID。**打开设备时就可以获得ID**，并在随后的mciSendCommand呼叫中使用。   
第二个参数是前缀为MCI的常数，这些称为MCI命令消息。RECORD2展示了其中的七个：MCI_OPEN、MCI_RECORD、MCI_STOP、MCI_SAVE、MCI_PLAY、MCI_PAUSE和MCI_CLOSE。    
dwFlags参数通常由0或者多个位旗标常数组成，这些通常用来表示不同的选项。一些选项是某个命令消息所特有的，而另一些对所有的消息都是通用的。   
dwParam参数通常是指向一个数据结构的长指针，该结构表示选项以及由设备获得的信息。许多MCI消息都与数据结构有关，而且这些数据结构对于消息来说都是唯一的。    
如果mciSendCommand函数调用成功则传回0，否则传回错误代码。要告知使用者错误，可用函数获得描述错误的文字字符串：    
```c
mciGetErrorString (error, szBuffer, dwLength)    
```     
按下「Record」按钮后，RECORD2的窗口消息处理程序就收到一个WM_COMMAND消息，其中wParam等于IDC_RECORD_BEG。   
RECORD2从打开设备开始，包括设定MCI_OPEN_PARMS结构的字段，并用MCI_OPEN命令消息呼叫mciSendCommand。录音时，lpstrDeviceType字段设定为字符串「waveaudio」以说明设备型态，lpstrElementName字段设定为长度为0的字符串。MCI驱动程序使用内定的取样频率和取样精确度，但是可以用MCI_SET命令进行修改。   
录音程序中，声音数据先储存在硬盘上的临时文件中，最后再转化成标准的波形文件。播放录制的声音时，MCI使用波形文件中定义的取样频率和取样精确度。    
要开始录音，RECORD2就调用mciSendCommand，以MCI_RECORD命令消息和MCI_WAVE_RECORD_PARMS数据结构为参数。另外也可以将此结构（并使用表示这些字段已设定的位旗标）的dwFromz和dwTo字段进行设定，以便将声音插入现有的波形文件，其文件名在MCI_OPEN_PARMS结构的lpstrElementName字段指定。内定状态下，任何新的声音都插入在现有文件的开始位置。     
录音结束按下「End」按钮来停止录音，产生一个WM_COMMAND消息，wParam为IDC_RECORD_END。响应该消息时，将调用mciSendCommand三次：   
1 MCI_STOP命令消息用于停止录音。  
2 MCI_SAVE命令消息用于把临时文件中的声音数据传递到MCI_SAVE_PARMS结构中指定的文件（「record2.wav」）。  
3 MCI_CLOSE命令消息用于删除所有的临时文件、释放已经建立的内存块并关闭设备。    
...
按下第二个「End」按钮也可以停止播放。这时产生wParam等于IDC_PLAY_END的WM_COMMAND消息。响应时，窗口消息处理程序将调用mciSendCommand两次：第一次使用MCI_STOP命令消息；第二次使用MCI_CLOSE命令消息来关闭设备。    
现在有一个问题：虽然可以通过按下「End」按钮来手工终止播放，但如果播放整个文件，程序如何知道文件什么时候播放完？这是MCI通知消息的任务（前面的MCI_NOTIFY标记）。    
之前带有MCI_RECORD和MCI_PLAY消息来调用mciSendCommand时，包含了MCI_NOTIFY标记，并将数据结构的dwCallback字段设定为程序窗口句柄。这样就可以产生一个通知消息，称为MM_MCINOTIFY，并在某些环境下传递给窗口消息处理程序。消息参数wParam是一个状态代码，而lParam是设备ID。      
带有MCI_STOP或者MCI_PAUSE命令消息来呼叫mciSendCommand时，将接收到一个MM_MCINOTIFY消息，其中wParam等于MCI_NOTIFY_ABORTED。当您按下「Pause」按钮或者两个「End」按钮中的一个时，就会出现这种情况。由于对这些按钮已进行过适当的处理，所以RECORD2可以忽略这种情况。   
播放时，会在声音文件结束后接收到MM_MCINOTIFY消息，其中wParam等于MCI_NOTIFY_SUCCESSFUL。这种情况下，窗口消息处理程序给自己发送一个WM_COMMAND消息，其中wParam等于IDC_PLAY_END，来仿真使用者按下「End」按钮。然后窗口消息处理程序作出正常响应：停止播放，关闭设备。    
录音时，如果用于储存临时文件的硬盘空间不够，将会接收一个MM_MCINOTIFY消息，其中wParam等于MCI_NOTIFY_SUCCESSFUL（虽然有点不合逻辑）。响应时，窗口消息处理程序给自己发送一个WM_COMMAND消息，其中wParam等于IDC_RECORD_END，然后与正常情况下一样：停止录音、储存文件并关闭设备。    
### MCI命令字符串的方法 
Windows的多媒体接口**曾经**包含函数mciExecute，其语法如下：   
```c
bSuccess = mciExecute (szCommand) ;   
```    
其中唯一的参数是MCI**命令字符串**。函数返回布尔值－如果调用成功，则返回非0值，否则返回0。在功能上，mciExecute函数相同于调用后三个参数为NULL或0的mciSendString（TESTMCI中使用的依据字符串的MCI函数），然后在发生错误时调用mciGetErrorString和MessageBox。    
虽然mciExecute不再是API的一部分，但是RECORD3中的数字录音机中使用了这个函数。和RECORD2一样，RECORD3程序也使用RECORD1中的资源描述档RECORD.RC和RESOURCE.H。   
**参考RECORD3**    
### 波形声音文件格式
### 迭加合成实验
参考ADDSYNTH
