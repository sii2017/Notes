...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInsatance, PSTR szCmdLine, int iCmdShow)
{
...
void Show(HWND hwnd, HDC hdc, int xText, int yText, int iMapMode, TCHAR * szMapMode)
{
	...
	DPtoLP(hdc, (PPOINT)&rect, 2);			//由设备转为逻辑

	RestoreDC(hdc, -1);
	TextOut(hdc, xText, yText, szBuffer, 
		wsprintf(szBuffer, TEXT("%-20s %7d %7d %7d %7d"),szMapMode,
		rect.left, rect.right, rect.top, rect.bottom));				//这里显示的是逻辑坐标，因此，第一个是正的后面都是负的 
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		hdc= GetDC(hwnd);
		SelectObject(hdc, GetStockObject(SYSTEM_FIXED_FONT));		//切换为固定间距的字体
	...
	case WM_PAINT:
		...
		Show(hwnd, hdc, 1, 6, MM_LOENGLISH, TEXT("LOENGISH(.01in)"));
