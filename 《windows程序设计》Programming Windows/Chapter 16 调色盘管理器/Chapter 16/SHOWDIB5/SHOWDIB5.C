...
本程序通过hPalette= CreateHalftonePalette(hdc):获取了windows API的通用调色盘
*/
#include <windows.h>
#include "PackeDib.h"
#include "resource.h"
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow)
{
	...
	hwnd= CreateWindow(szAppName, TEXT("Show DIB #5: Native Palette"), WS_OVERLAPPEDWINDOW,
...
LRESULT CALLBACK WndProc (HWND hwnd, UINT message, WPARAM wParam,LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		hdc= GetDC(hwnd);
		hPalette= CreateHalftonePalette(hdc)://获取通用调色盘
		ReleaseDC(hwnd, hdc);
		return 0 ;
	case WM_SIZE:
