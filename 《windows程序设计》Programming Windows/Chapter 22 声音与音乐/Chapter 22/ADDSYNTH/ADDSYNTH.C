/*
书上说，这里没有给出附加文件ADDSYNTH.H，因为它包含几百行令人讨厌的叙述，您将在本书附上的光盘上找到它。
然而我并没有光盘，因此这个程序无法编译。
*/
#include <windows.h>
#include <math.h>
	...
	for(iSmp= 0; iSmp<iNumSamples; iSmp++)
	{
		...
		for(iPrt= 0; iPrt<ins.iNumPartials; iPrt++)
		{
			...
			for(i=0; i<ins.pprt[iPrt].iNumAmp- 1; i++)
			{
				...
				{
					dFrac= (double)(iMsecTime- ins.pprt[iPrt].pEnvAmp[i].iTime)/
						(ins.pprt[iPrt].pEnvAmp[i+1].iTime-
						ins.pprt[iPrt].pEnvAmp[i].iTime);

					dAmp= dFrac*ins.pprt[iPrt].pEnvAmp[i+1].iValue+ (1-dFrac)*
						ins.pprt[iPrt].pEnvAmp[i].iValue;
					break;
				}
			}
			...
			for(i=0; i<ins.pprt[iPrt].iNumFrq-1; i++)
			{
				...
				{
					dFrac= (double)(iMsecTime- ins.pprt[iPrt].pEnvFrqpi].iTime/
			...
			dComp+= dAmp*SinGenerator(dFrq, dAngle+ iPrt);
		}
...
void TestAndCreateFile(HWND hwnd, INS ins, TCHAR* szFileName, int idButton)
{
	...
	{
		if(MakeWaveFile(inss, szFileName))
			EnableWindow(GetDlgItem(hwnd, idButton), TRUE);
		else
		{
...
int WINAPI WinMain(HINSTANCE hInstance, HISNTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow)
{
