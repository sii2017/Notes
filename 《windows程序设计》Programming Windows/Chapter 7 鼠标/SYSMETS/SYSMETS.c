...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		cxCaps= (tm.tmPitchAndFamily&1?3:2)*cxChar/2;//如果是等宽，那么大写的也是cxChar，如果不是等宽则是1.5倍宽
		cyChar= tm.tmHeight+tm.tmExternalLeading;

		ReleaseDC(hwnd, hdc);	//Save the width of the three colums

		iMaxWidth= 40*cxChar + 22*cxCaps;
	case WM_SETTINGCHANGE://系统策略设置发生改变后，顶层窗口会收到
		SystemParametersInfo(SPI_GETWHEELSCROLLLINES, 0, &ulScrollLines, 0);//SPI_GETWHEELSCROLLLINES可获取滚动的行数
		if(ulScrollLines)//如果取到了
			iDeltaPerLine= WHEEL_DELTA / ulScrollLines;//获取单次滚动的delta数值
		else
			iDeltaPerLine= 0;
		return 0;
	case WM_SIZE:
	...
	case WM_HSCROLL:
		...
		return 0;
	case WM_KEYDOWN:	//接收消息并转发消息给程序的滚动条case
		switch(wParam)
		{
		case VK_HOME:
		...
		return 0;
	case WM_MOUSEWHEEL:
		if(iDeltaPerLine==0)//如果等于0说明“可能不支持滚轮”
			break;
		iAccumDelta += (short)HIWORD(wParam);	//120 or -120， 如果是120，每一行是40，那么滚轮一下能滑3次
		while(iAccumDelta>=iDeltaPerLine)
		{
			SendMessage(hwnd, WM_VSCROLL, SB_LINEUP, 0);
			iAccumDelta -= iDeltaPerLine;	//iDeltaPerLine代表卷动一次的值，循环直至iAccumDelta归零
		}
		while(iAccumDelta<= -iDeltaPerLine)
		{
			SendMessage(hwnd, WM_VSCROLL, SB_LINEDOWN, 0);
...
/* 总结
获取每一行需要的delta数，本文是40。然后划一下是120，能卷动三行。
*/