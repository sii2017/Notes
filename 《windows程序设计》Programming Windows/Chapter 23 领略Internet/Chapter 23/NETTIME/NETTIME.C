/*
需要在附加库中加入ws2_32.lib来支持各种与socket有关的函数
该程序从网站获取标准时间（虽然我获取失败了，不知道是不是由于墙的关系）
然后更新系统时间以及显示在dialog中。   
*/
#include <windows.h>
#include "resource.h"
...
{
	//正常创建窗口，窗口没有设立背景颜色。并且创建一个dialog
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_SETFOCUS:
		SetFocus(hwndModeless);	//总是将焦点给与dialog
		return 0;
	case WM_DESTROY:
		PostQuitMessage(0);
		return 0;
...
BOOL CALLBACK MainDlg(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		{
		case IDC_SERVER:	//左边的按键Select Server
			DialogBoxParam(hInst, TEXT("Servers"), hwnd, ServerDlg, (LPARAM)szIPAddr);
			return TRUE;
		case IDOK:	//正当中的按键 Set Correct Time，这个按钮是程序最开始的操作。
			//Call "WSAStartup" and display decscription text
			if(iError= WSAStartup(MAKEWORD(2,0), &WSAData))		//socket的启动命令
			{
			...
			//Call "WSAAsyncSelect" 有些函数具有“阻碍性”即从互联网获取数据之前不会将控制权传回程序导致程序卡住，而这个函数可以强制使那些程序“失去阻碍性”，总是能将控制权还给程序。而获得的数据以消息WM_SOCKET_NOTIFY来返回
			...
			//"connect" will return SOCKET_ERROR beacuse even if it succeeds, it will require blocking
			//The following only reports unexpected errors.
			...
			//The result of the "connect" call will be reported through the WM_SOCKET_NOTIFU message.
			//Set tiemr and change the button to "Cancel"
	...
	case WM_SOCKET_NOTIFY:
		...
		wError= WSAGETSELECTEVENT(lParam);	//ie, HIWORD

		//Process 2 events specified in WSAAsyncSelect
		switch(wEvent)
		{
		...
		case FD_CONNECT:
			...
			//of WSAEWOULDLOCK and an event of FD_READ
		...
		case FD_READ:
			...
			//Get the tiem and swap the bytes
			iSize= recv(sock, (char*)&ulTime, 4, 0);
			ulTime= ntohl(ulTime);	//调整字节顺序，以便Intel微处理器能够处理。
...
void ChangeSystemTime(HWND hwndEdit, ULONG ulTime)
{
	...
	SystemTimeToFileTime(&stNew, &ftNew);	//将SYSTEMTIME转化为FILETIME结构
	...
	{
		GetLocalTime(&stNew);
		FormatUpdatedTime(hwndEdit, &stOld, &stNew);	//这个函数用于在dialog中显示时间（需要格式化一下时间的值）
	}
	else
...
void FormatUpdatedTime(HWND hwndEdit, SYSTEMTIME* pstOld, SYSTEMTIME* pstNew)
{
	...
	GetDateFormat(LOCALE_USER_DEFAULT, LOCALE_NOUSEROVERRIDE| DATE_SHORTDATE, pstOld, 
		NULL, szDateOld, sizeof(szDateOld));
	...
	GetDateFormat(LOCALE_USER_DEFAULT, LOCALE_NOUSEROVERRIDE| DATE_SHORTDATE, pstNew,
		NULL, szDateNew, sizeof(szDateNew));
	GetTimeFormat(LOCALE_USER_DEFAULT, LOCALE_NOUSEROVERRIDE| TIME_NOTIMEMARKER|
		TIME_FORCE24HOURFORMAT, pstNew, NULL, szTimeNew, sizeof(szTimeNew));
	EditPrintf(hwndEdit, TEXT("System date and time successfully changed")
		TEXT("from\r\n\t%s, %s.%^03i to\r\n\t%s, %s.%03i."), szDateOld, szTimeOld, 
		pstOld->wMilliseconds, szDateNew, szTimeNew, pstNew->wMilliseconds);
}
