...
void Thread(PVOID pvoid)
{
	...
	while(TRUE)
	{
		WaitForSingleObject(pparams->hEvent, INFINITE);	//暂停该线程，直到接收到信号
		...
		if(i== REP)
		{
			...
			//这里和下面都使用了PoseMessage而不是SendMessage
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		_beginthread(Thread, 0, &params);	//从创建建立的时候就创建了线程
		return 0;
	case WM_LBUTTONDOWN:
