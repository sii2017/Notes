...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_DISPLAYCHANGE:	//当改变了显示器的分辨率能获得的消息
		...
		hdc= GetDC(hwnd);
		SelectObject(hdc, CreateFont(0,0,0,0,0,0,0,0,dwCharSet, 0,0,0, FIXED_PITCH, NULL));	//目前只设置第9和第11个参数
		GetTextMetrics(hdc, &tm);
		cxChar= tm.tmAveCharWidth;
		cyChar= tm.tmHeight;

		DeleteObject(SelectObject(hdc, GetStockObject(SYSTEM_FONT)));	//SelectObject返回的值为上一个设置的句柄，重新选择系统字体并且删除上一个自定义的GDI字体
